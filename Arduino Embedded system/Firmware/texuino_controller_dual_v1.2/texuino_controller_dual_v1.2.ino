...
void processCommand(String cmd)
{
  ...
  if (cmd.length() > 0)
  {
    ...
    if (cmd.indexOf("+IPD,") >= 0)
    {
      ...
      {
        //feedback = "FBB:(" + String(getInputs()) + ":" + String(relayByte) + ":" + String(getInterupts()) + ":)";
        streamWrite(id, feedback, true);
      }
      //------------------------------------------------------------------END--------------------------------------------------------------------\\     
...
String constructFeedback()
{
  String fb = "FBB:(" + String(getInputs()) + ":" + String(relayByte) + ":" + String(getInterupts()) + ":)";
  return fb;
}

void updateFeedback()
{
  String fb = constructFeedback();
  if (fb != feedback)
  {
    feedback = fb;
    streamWrite("0", feedback, true);
    for (int i = 0; i < 5; i++)
    {
      if (connectionStatus[i] == CONNCTD)
      {
        streamWrite(String(i), feedback, true);
      }
    }
  }
}  
...
void setServer(String state, String port)
{
  sendAtCommand("AT+CIPSERVER=" + state + "," + port, 150);
}

void multipleConnections(boolean condition)
{
  if (condition)
  {
    sendAtCommand("AT+CIPMUX=1", 150);
  }
  else
  {
    sendAtCommand("AT+CIPMUX=0", 150);
  }
}
...
{
  sendAtCommand("AT+CWMODE=" + mode, 100);
}
...
{
  sendAtCommand("AT+CIPSTATUS", 100);
}

void closeConnection(String connectionId, int delayTime)
{
  delay(delayTime);
  sendAtCommand("AT+CIPCLOSE=" + connectionId, 100);
}

void configureNetwork(String ssid, String password, String channel, String protection)
{
  String quote = "\"";
  ssid = quote + ssid + quote;
  password = quote + password + quote;
  String cmd = "AT+CWSAP=" + ssid + "," + password + "," + channel + "," + protection;

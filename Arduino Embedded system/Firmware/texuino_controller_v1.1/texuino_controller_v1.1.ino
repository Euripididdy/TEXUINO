...
void processCommand(String cmd)
{
  ...
  {
    // CONNECTION WHOIS
    if (cmd.indexOf("0,CONNECT") >= 0) //or "Link"
    {
      connectionStatus = 1;
      //sendInfoMessage("connected", true);
      //streamWrite("connected", true);
    }
    if (cmd.indexOf("0,CLOSED") >= 0) //or "Unlink"
    {
      connectionStatus = 0;
    ...
    if ((cmd.indexOf("CNET") >= 0) && (cmd.indexOf("<") >= 0) && (cmd.indexOf(">") >= 0) && (cmd.indexOf("{") >= 0) && (cmd.indexOf("}") >= 0))
    {
      ...
      {
        sendInfoMessage("Network configuration successfully changed", true); 
        setupModule();
      }
      else
      {
        sendInfoMessage("Error in network configuration change!", true); 
      }
    }

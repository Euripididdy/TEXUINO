...
void processCommand(String cmd)
{
  ...
  if (cmd.length() > 0)
  {
    ...
    if (cmd.indexOf("+IPD,") >= 0)
    {
      ...
      {
        streamWrite(id, String(getInputs()) + ":" + String(relayByte) + ":" + String(getInterupts()) + ":", true);
      }
      //------------------------------------------------------------------END--------------------------------------------------------------------\\     
...
boolean configureNetwork(String name, String password, String channel, String protection)
{
  ...
  String cmd = "CWSAP=" + name + "," + password + "," + channel + "," + protection;
  return sendAtCommand("CWSAP=" + cmd, 100);
}
